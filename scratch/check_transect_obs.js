...
async function checkTransectObs() {
  const { data, error } = await supabase.from('transect_observations').select('*').limit(1);
  if (error) {
    console.error('Error:', error.message);
  } else {
    console.log('Sample transect_observation:', data[0]);
