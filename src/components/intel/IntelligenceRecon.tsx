                 ...
                 {speciesData.map(s => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 flex-shrink-0 shadow-sm">
                      {getSpeciesPortrait(s.name) ? (
                        <img 
                          src={getSpeciesPortrait(s.name)!} 
                    ...
                    <div className="flex-1 text-left">
                       ...
                       <div className="text-[9px] font-bold text-slate-400 font-mono">{s.count} OBS</div>
                    </div>
...
/div>
      ...
      {/* ── Species Spotlig        {selectedSpecies ? (
          <motion.div 
            key="profile"
                      ...
                      <tbody>
                        {speciesProfile?.observations.slice(0, 4).map((o, i) => (
                          <tr key={o.id} className={`border-b border-slate-50 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                            ...
                            <td className="px-4 py-3 text-[10px] font-bold text-slate-400">{o.observer}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
